...
interface ServiceCardProps {
  service: Service;
  index: number;
  onClick: (service: Service) => void; // Add onClick prop
...
const ServiceCard: React.FC<ServiceCardProps> = ({
  ...
  return (
    <motion.div
      ...
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.5, delay: index * 0.1 }}
      className="relative overflow-hidden transition-all duration-300 bg-white rounded-lg shadow-lg group hover:shadow-xl cursor-pointer"
      onClick={() => onClick(service)} // Call the onClick prop with the service object
    >
      <div className="relative p-6">
        <div className="absolute inset-0 z-0 transition-opacity duration-300 opacity-0 bg-gradient-to-b from-primary-50 to-primary-100 group-hover:opacity-100"></div>
...
/*
  isExpanded prop and detailed view logic removed as per current requirement
  onClose prop removed as per current requirement
  Animation variants for expanded state removed
  Conditional rendering for detailed view inside card removed
